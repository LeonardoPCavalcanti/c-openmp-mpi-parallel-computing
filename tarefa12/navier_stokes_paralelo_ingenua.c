...
int main(int argc, char *argv[]) {
    ...
    // u: velocidade horizontal, v: velocidade vertical
    // u_novo/v_novo: grades temporárias para o próximo passo no tempo
    ...
    for (int passo = 0; passo < NUMERO_DE_PASSOS; passo++) {
        ...
        // Região 0.1: Uma equipe de threads é criada e destruída AQUI a cada passo.
        #pragma omp parallel for
        for (int i = 1; i < LARGURA-1; i++) {
            for (int j = 1; j < ALTURA-1; j++) {
                // Calcula a difusão (como a velocidade se espalha)
        ...
        // Região 0.2: OUTRA equipe de threads é criada e destruída AQUI.
        ...
        // Truque para evitar cópia de dados: apenas trocamos os ponteiros das grades
        double **temp_u = u; u = u_novo; u_novo = temp_u;
        double **temp_v = v; v = v_novo; v_novo = temp_v;
    }
    
    double tempo_final = omp_get_wtime();
    printf("Tempo de execução (versão simples): %.6f segundos\n", tempo_final - tempo_inicial);
