...
// (As funções inicializar_velocidades e aplicar_contorno são idênticas às da versão OMP Simples)
...
int main(int argc, char **argv){
    ...
    // Alocação de memória (idêntica à anterior)
    ...
    for (int passo_atual = 0; passo_atual < NUMERO_DE_PASSOS; passo_atual++){
        #pragma omp parallel
        {
            // collapse(2): Trata os dois laços aninhados como um só para paralelização.
    ...
    printf("[OMP collapse+static] Tempo de execução: %.6f s\n", tempo_fim - tempo_fim);

    // Liberação de memória
    for (int x=0; x<LARGURA_GRID; x++){ free(velocidade_x[x]); free(velocidade_y[x]); free(proxima_velocidade_x[x]); free(proxima_velocidade_y[x]); }
    free(velocidade_x); free(velocidade_y); free(proxima_velocidade_x); free(proxima_velocidade_y);
    return 0;
}