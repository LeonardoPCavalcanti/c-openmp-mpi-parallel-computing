...
int main(int argc, char **argv){
    ...
    for (int passo_atual = 0; passo_atual < NUMERO_DE_PASSOS; passo_atual++){
        #pragma omp parallel
        {
            // O tamanho dos pedaços de trabalho distribuídos começa grande e diminui
            // exponencialmente até o tamanho mínimo 'chunk'.
            if (chunk > 0){
            ...
            } else { // Usa o chunk padrão do OpenMP
    ...
    if (chunk > 0) printf("[OMP collapse+guided, chunk=%d] Tempo: %.6f s\n", chunk, tempo_fim - tempo_inicio);
    else         printf("[OMP collapse+guided] Tempo: %.6f s\n", tempo_fim - tempo_inicio);
